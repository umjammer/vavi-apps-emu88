 ...
 * <p>
 * TODO let memory as {@link Device}
 * </p>
 *
...
public class PC88 {
    ...
    private static final Logger logger = getLogger(PC88.class.getName());
...
//      romDao.read("/roms/romsub.bin", ROM_SUB, 0x800);
        ...
        logger.log(Level.INFO, "sw1: %02x".formatted(sw1));
        logger.log(Level.INFO, "sw2: %02x".formatted(sw2));
    }
    ...
    /**
     * dip switch 1
     * create a bit with the following ON/OFF, invert it, and set it.
     * <pre>
     * SW1                      ON                 OFF
     *
...
//logger.log(Level.DEBUG, "use default: rom " + (rom4th + 1) + " does not exists");
                            mapping.base = ROM_N88;
                            mapping.pointer = address;
                        }
                    } else {
...
//logger.log(Level.DEBUG, "4th rom: " + (rom4th + 1));
                    }
                    break;
        ...
        /** 31h */
        private int uip1() {
            return !uiop.getPort1() ? 0x40 : 0x00;
        }

        /** 31h */
        private int uip2() {
            return !uiop.getPort2() ? 0x80 : 0x00;
        }

        /** 40h */
        private int exton() {
            return (sw2 & 0x40) != 0 ? 0x00 : 0x08;
        }

        /** 40h */
        private int vrtc() {
            return intc.getVrtc() ? 0x20 : 0x00;
        }

        /** 40h */
        private int dcd() {
            return false ? 0x00 : 0x40;
        }

        /** 40h */
        private int busy() {
            return printer.isBusy() ? 0x01 : 0x00;
        }

        /** 40h */
        private int shg() {
            return true ? 0x00 : 0x02;
        }

        /** 40h out */
        private boolean pstb() {
